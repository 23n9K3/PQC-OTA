...
void ota_sha256_update(ota_sha256_ctx_t *ctx, const uint8_t *data, size_t length)
{
    ...
    while (length > 0U) {
        uint32_t copy_len = 64U - ctx->buffer_len;
        if (copy_len > length) {
            copy_len = (uint32_t)length;
        }
...
void ota_sha256_final(ota_sha256_ctx_t *ctx, uint8_t hash[OTA_SHA256_SIZE])
{
    ...
    for (uint32_t i = 0U; i < 8U; i++) {
        ctx->buffer[63U - i] = (uint8_t)(bit_count >> (i * 8U));
    }
    sha256_transform(ctx, ctx->buffer);
...
void ota_sha256_compute(const uint8_t *data, size_t length, uint8_t hash[OTA_SHA256_SIZE])
{
    ...
    ota_sha256_init(&ctx);
    ota_sha256_update(&ctx, data, length);
...
{
    uint8_t diff = 0U;
    for (uint32_t i = 0U; i < OTA_SHA256_SIZE; i++) {
